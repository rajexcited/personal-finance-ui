...
export const DeleteAccountSection: FunctionComponent<DeleteAccountSectionProps> = (props) => {
  ...
  useEffect(() => {
    const useEffectLogger = getLogger("useEffect.dep[actionState]", fcLogger);
    if (actionState === ActionState.NoAction) {
      const logger = getLogger("actionState.noaction", useEffectLogger);
      setPassword((prev) => {
  ...
  const content = useMemo(() => {
    ...
    return (
      <form onInvalid={onFormInvalidHandler} onChange={onFormChangeHandler}>
        <div>
          <span>To confirm deletion request please authenticate your credentials.</span>
        </div>
        <Input
          id="emailId"
          type="email"
        ...
        <article className="message is-warning">
          <div className="message-body">
            <p>
              ...
              Deactivating your account will restrict access to your data. You will have read-only access and the ability to download your data. Your account
              will be permanently deleted after 6 months, and recovery will not be possible. To proceed, please click the "De-activate" button below.
            </p>
          </div>
        </article>
      </form>
    ...
    <section>
      <div className="columns">
        <div className="column dwp is-4">
          ...
          <button className="button is-dark" onClick={onClickRequestHandler} disabled={props.readOnly || actionState !== ActionState.NoAction}>
            Delete Account
          </button>
        </div>
      </div>
