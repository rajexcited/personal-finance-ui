...
export const ExpenseItemCard: FunctionComponent<ExpenseItemCardProps> = (props) => {
  ...
  if (props.details.belongsTo === ExpenseBelongsTo.Income) {
    ...
    belongsToIconTip = (
      <span className="icon has-text-info tooltip is-tooltip-top" data-tooltip={belongsTo}>
  ...
  const updateExpenseAction = (
    <button
      ...
      onClick={onClickEditExpenseHandler}
  ...
  const removeExpenseAction = (
    <button
      className="button is-link is-active"
      onClick={onClickTrashExpenseHandler}
  ...
  const viewReceiptsAction = (
    <button
      className="button is-link is-active"
      onClick={onClickShowReceiptsHandler}
  ...
  const addRefundAction = (
    <button
      className="button is-link is-active"
      onClick={onClickAddRefundHandler}
          ...
          <div className="card-header-title">
            <div className="card-header-icon" onClick={onClickBodyToggleHandler}>
              <div className="columns is-desktop">
                ...
                <div className="column">
                  <div className="buttons">{actions.map((ae) => ae)}</div>
                </div>
              </div>
            </div>
          </div>
                ...
                {verifiedDate.length > 1 && (
                  <div className="column">
                    ...
                    <div className="level is-mobile">
                      <span className="level-item is-narrow" {...testAttributes("outvalue")}>
                        {verifiedDate}
                      </span>
                ...
                <div className="column">
                  ...
                  <div className="tags" {...testAttributes("outvalue")}>
                    {props.details.tags.map((tag) => (
                      <span className="tag is-link" key={tag + "-tag-key"}>
                        {tag}
                      </span>
                    ))}
                    {props.details.tags.length === 0 && <span>-</span>}
                  </div>
                </div>
                <div className="column">
                  ...
                  <div className="tags" {...testAttributes("outvalue")}>
                    {props.details.personIds.map((pid) => (
                      <span className="tag is-link" key={pid + "-tag-key"}>
                        {sharePersonMap[pid].displayText}
                      </span>
                    ))}
                    {props.details.personIds.length === 0 && <span>-</span>}
                  </div>
                </div>
                <div className="column">
