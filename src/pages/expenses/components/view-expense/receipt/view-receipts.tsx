...
export const ViewReceipts: FunctionComponent<ViewReceiptsProps> = (props) => {
  ...
  const onShowCarouselItemHandler = useCallback(
    (carousel: bulmaCaraosel) => {
  ...
  useEffect(() => {
    ...
    if (props.receipts.length > 0) {
      ...
      receiptService.downloadReceipts(props.receipts).then((downloadReceipts) => {
        logger.info("receipts are downloaded. downloadReceipts =", downloadReceipts, ". time taken:", getDiffSeconds(startTime), "sec");
        const error = downloadReceipts
          .map((dr) => dr.status === "fail" && dr.error)
          .filter((dr) => dr)
          .join("\n\n");
        ...
        const receiptsWithUrl = downloadReceipts
          .map((dr) => {
        ...
        <div className="modal-card is-fullscreen">
          <section className="modal-card-body">
            <div className="modal-close">
              <button className="delete" type="button" onClick={onClickModalCloseHandler} {...testAttributes("receipt-close-action")}></button>
            </div>
            {isActiveItemImage && (
              <div className="modal-card-header-actions">
                ...
                <button
                  className="button is-white is-inverted image-zoomOut"
                  type="button"
                  onClick={onClickZoomOutHandler}
                  {...testAttributes("image-zoomout")}
                >
            ...
            {isReceiptLoading && (
              <article className="message is-warning" {...testAttributes("receipt-loading-wait")}>
                <div className="message-body">Please wait receipt is loading.</div>
              </article>
            )}
            <div className="carousel" ref={carouselRef}>
              {receipts.map((rct: ReceiptProps, ind: number) => (
                <div className={`item-${ind + 1}`} key={rct.id} {...testAttributes("carousel-item", "receipt-name", rct.name)}>
                  <div className="fullscreen-image-container">
                    ...
                    {rct.contentType === ReceiptType.PDF && (
                      <embed src={rct.url} type={rct.contentType} height={"99%"} width={"93%"} {...testAttributes("pdf", "receipt-type", rct.contentType)} />
                    )}
                  </div>
                </div>
              ))}
              {receipts.length === 0 && (
                <div className="item-1" {...testAttributes("dummy-item")}>
                  <span> Dummy item One </span>
                </div>
              )}
            </div>
          </section>
