...
export const UpdateIncome: FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    useEffectLogger.debug(
      ...
      navigation.location,
      "; shortPath: ",
      getShortPath("updateIncome", loaderData.data?.incomeDetail?.id)
    );
    if (navigation.state === "submitting") {
      setSubmitLabel("Updating...");
    } else if (navigation.state === "loading" && !navigation.location?.pathname.endsWith(getShortPath("updateIncome", loaderData.data?.incomeDetail?.id))) {
    ...
    <>
      {errorMessage && (
        <Animated
          ...
          {...testAttributes("update-income-error-message")}
        >
