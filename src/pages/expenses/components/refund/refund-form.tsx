...
export const PurchaseRefundForm: FunctionComponent<PurchaseRefundFormProps> = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.purchaseDetails) {
      ...
      logger.debug(
        "refund pymtAccId =",
        props.refundDetails?.paymentAccountId,
        " purchase pymtAccId =",
        props.purchaseDetails.paymentAccountId,
        "will configure selected pymtAcc if possible."
      );
    ...
    logger.debug(
      ...
      ", props.refundDetails?.paymentAccountId =",
      props.refundDetails?.paymentAccountId,
      ", props.refundDetails?.paymentAccountName =",
      props.refundDetails?.paymentAccountName
    );
    ...
    logger.info(
      "props.sharePersons =",
      props.sharePersons,
      ", mySourceSharePersonTagItems =",
      mySourceSharePersonTagItems,
      ", props.details?.personIds =",
      props.refundDetails?.personIds
    );
  ...
  const loadMorePurchaseList = async () => {
    ...
    setDropdownPurchaseDetailList((prev) => {
      ...
      Object.keys(newDdMap)
        .filter((nid) => !(nid in ddMap))
        .forEach((nid) => {
      ...
      <form onSubmit={onSubmitHandler}>
              ...
              <div className="block">
                ...
                {!!selectedPurchaseReference?.taggingPersons.length && (
                  <ul {...testAttributes("outvalue")}>
                    {selectedPurchaseReference.taggingPersons.map((sp, index) => (
                      <li key={index}>{sp}</li>
                    ))}
                  </ul>
                )}
                {!selectedPurchaseReference?.taggingPersons.length && <span {...testAttributes("outvalue")}>-</span>}
              </div>
            </ViewDialog>
          </div>
        </div>
        <div className="columns">
          <div className="column">
            <div className="columns">
              <div className="column">
                ...
                <Animated
                  ...
                  isPlayIn={!!selectedPurchaseReference}
                  animateOnMount={true}
                  isVisibleAfterAnimateOut={false}
                >
            ...
            <div className="columns">
              <div className="column">
                <TextArea id="refund-desc" label="Description: " rows={2} value={description} onChange={setDescription} key="refund-desc" maxlength={150} />
              </div>
            </div>
            <div className="columns">
              <div className="column">
              ...
              <div className="column">
                <div className="my-1 py-1">
                  <Calendar
                    ...
                    onSelect={(range) => {
                      setRefundDate((prev) => range.start || prev);
                    }}
                  />
                </div>
              </div>
        ...
        <div className="columns footer-form-actions is-display-mobile">
          <div className="column">
            <div className="buttons">
              <button className="button is-dark is-fullwidth is-large" type="submit" {...testAttributes("submit-refund")}>
                <span className="px-2-label">{props.submitLabel}</span>
              </button>
              <button className="button is-light" type="button" onClick={onCancelHandler} {...testAttributes("cancel-refund")}>
                <span className="px-2-label">Cancel</span>
              </button>
            </div>
          </div>
        </div>
        <div className="columns footer-form-actions is-hidden-mobile">
          <div className="column">
            <div className="buttons">
              <button className="button is-light" type="button" onClick={onCancelHandler} {...testAttributes("cancel-refund")}>
                <span className="px-2-label">Cancel</span>
          ...
          <div className="column">
            <div className="buttons is-centered">
              <button className="button is-dark is-large" type="submit" {...testAttributes("submit-refund")}>
                <span className="px-2-label">{props.submitLabel}</span>
              </button>
            </div>
          </div>
