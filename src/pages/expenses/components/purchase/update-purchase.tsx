...
export const UpdatePurchase: FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    useEffectLogger.debug(
      ...
      navigation.location,
      "; shortPath: ",
      getShortPath("updatePurchase", loaderData.data?.purchaseDetail?.id)
    );
    if (navigation.state === "submitting") {
      setSubmitLabel("Updating...");
    } else if (navigation.state === "loading" && !navigation.location?.pathname.endsWith(getShortPath("updatePurchase", loaderData.data?.purchaseDetail?.id))) {
