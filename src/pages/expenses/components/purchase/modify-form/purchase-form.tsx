...
export const PurchaseForm: FunctionComponent<PurchaseFormProps> = (props) => {
  ...
  useEffect(() => {
    ...
    logger.info(
      "props.purchaseTypes =",
      props.purchaseTypes,
      ", myDropdownPurchaseTypeItems =",
      myDropdownPurchaseTypeItems,
      ", props.details?.purchaseTypeId =",
      props.details?.purchaseTypeId,
      ", props.details?.purchaseTypeName =",
      props.details?.purchaseTypeName
    );
    ...
    logger.debug(
      ...
      props.details?.paymentAccountId,
      ", props.refundDetails?.paymentAccountName =",
      props.details?.paymentAccountName
    );

    if (props.details?.paymentAccountId && props.details.paymentAccountName) {
      let mySelectedPaymentAcc = ddPymtAccList.find(
        (pymtacc) => pymtacc.id === props.details?.paymentAccountId && pymtacc.content === props.details.paymentAccountName
      );
    ...
    <form onSubmit={onSubmitHandler}>
      <div className="columns">
        <div className="column">
          ...
          <div className="columns">
            <div className="column">
              <TextArea id="purchase-desc" label="Description: " rows={2} value={description} onChange={setDescription} key={"purchase-desc"} maxlength={150} />
            </div>
          </div>
          <div className="columns">
            <div className="column">
            ...
            <div className="column">
              <div className="my-1 py-1">
                <Calendar
                  ...
                  onSelect={(range) => {
                    setPurchaseDate((prev) => range.start || prev);
                  }}
                />
              </div>
            </div>
      ...
      <div className="columns footer-form-actions is-display-mobile">
        <div className="column">
          <div className="buttons">
            <button className="button is-fullwidth is-dark is-large" type="submit" {...testAttributes("submit-purchase")}>
