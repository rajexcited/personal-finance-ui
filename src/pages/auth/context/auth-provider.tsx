...
import { purchaseService, incomeService, refundService } from "../../expenses/services";
...
const AuthContextProvider: FunctionComponent<AuthContextProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const useEffectLogger = getLogger("useEffect.dep[userDetails, expiringStatus]", fcLogger);
    let intervalId: number | undefined = undefined;

    const setAuthenExpire = () => {
      const logger = getLogger("setAuthenExpire", useEffectLogger);
      ...
      refundService.clearCache();
    };

    const setAuthenNotExpired = async () => {
      const logger = getLogger("setAuthenNotExpired", useEffectLogger);
    ...
    const setAboutToExpire = () => {
      const logger = getLogger("setAboutToExpire", useEffectLogger);
    ...
    if (!userDetails.isAuthenticated) {
      ...
      useEffectLogger.debug("user not authenticated. indicating scenario 1 (page load / public user) & 2 (logged out user) ");
      setAuthenExpire();
    } else {
      ...
      intervalId = window.setInterval(async () => {
        ...
        const logger = getLogger("authenticated.setInterval", useEffectLogger);
    ...
    return () => {
      useEffectLogger.debug("clearing old interval. will be configuring new interval in new use effect function call");
  ...
  return (
    ...
    >
      <Animated
        ...
        isPlayIn={expiringStatus === ExpireStatus.ExpiringSoon || expiringStatus === ExpireStatus.Expired}
          ...
          <div className="container mt-2 pt-2">
            <div className="notification-container px-5 mx-5">
              <div className="notification is-link is-light" {...testAttributes("expire-status-msg")}>
                ...
                {expiringStatus !== ExpireStatus.ExpiringSoon && expiringStatus !== ExpireStatus.Expired && <p>&nbsp;&nbsp;&nbsp;</p>}
              </div>
            </div>
          </div>
        }
